...
(async () => {
  ...
  try {
    ...
    // Click the row containing HAN
    const originClicked = await page.evaluate(() => {
      const divs = Array.from(document.querySelectorAll('div'));
      // Find the row containing 'HAN' and 'Ha Noi'
      const row = divs.find(d => d.innerText && d.innerText.includes('HAN') && d.innerText.includes('Ha Noi') && d.innerText.includes('Airport'));
      if (row) {
        row.click();
        return true;
      }
    ...
    console.log("Origin row clicked:", originClicked);
    await new Promise(r => setTimeout(r, 1500));

    // Take screenshot to see if dropdown closed and "From" shows "Ha Noi"
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '13_origin_row_clicked.png') });
