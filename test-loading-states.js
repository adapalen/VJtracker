...
(async () => {
  ...
  try {
    ...
    page.on('response', async (response) => {
      ...
      if (url.includes('flight') || url.includes('search') || url.includes('availability') || url.includes('api')) {
        ...
        try {
          ...
          console.log(`Content length: ${text.length}. Snippet: ${text.substring(0, 300)}`);
        } catch (e) {}
      }
    });
    ...
    // Sequential screenshots to track loading
    for (let sec = 5; sec <= 25; sec += 5) {
      console.log(`Waiting for ${sec}s mark...`);
      await new Promise(r => setTimeout(r, 5000));
      const currentUrl = page.url();
      console.log(`At ${sec}s, URL is: ${currentUrl}`);
      await page.screenshot({ path: path.join(ARTIFACT_DIR, `19_search_loading_${sec}s.png`) });
    }
